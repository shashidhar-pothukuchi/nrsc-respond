...
<?php include('copi_add.php'); ?>

                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                            <div class="alert alert-info">
                                <button type="button" class="close" data-dismiss="alert">&times;</button>
                                <strong><i class="icon-user icon-large"></i>&nbsp;co-principal investigator Table</strong>                            </div>
                            <thead>
                                <tr>
                                  <th colspan="6" style="text-align:center;"> CO-PRINCIPAL INVESTIGATOR DETAILS </th>
                                </tr>
                                <tr>
							...
							<?php
								require_once('db.php');
								$result = $conn->prepare("SELECT * FROM copi ORDER by id ASC");
								$result->execute();
							...
							?>
								...
								<td style="text-align:center; width:350px;">
									<a href="editcopi.php<?php echo '?id='.$id; ?>" class="btn btn-info">Edit</a>
								...
								<a href="deletecopi.php<?php echo '?id='.$id; ?>" class="btn btn-danger">Yes</a>								</div>
								</div>
								</div>
								</tr>
								<?php } ?>
